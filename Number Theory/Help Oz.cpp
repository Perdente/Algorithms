//https://www.hackerearth.com/practice/math/number-theory/basic-number-theory-1/practice-problems/algorithm/a-simple-task/description/
...
void SectumSempra()
{
	cin>>n;
	St s,ans;
	lp(i,n)cin>>x,s.insert(x);
	temp=*s.rbegin()-*s.begin();
	for(ll i=1;i*i<=temp;++i)
	{
		if(temp%i==0)ans.insert(i),ans.insert(temp/i);
	}
	for(auto it:ans)
	{
		flag=true;
		...
		for(auto i:s)
		{
			if(i%it!=temp)flag=false;
		}
		if(flag and it>1)cout<<it<<" ";
	}
	lol
}
